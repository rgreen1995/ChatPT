...
def render():
    ...
    if "nutrition_consultation_id" not in st.session_state:
        try:
            st.session_state.nutrition_consultation_id = create_consultation(
                st.session_state.user_id, consultation_type="nutrition"
            )
        ...
        initial_message = {"role": "assistant", "content": initial_content}
    ...
    if prompt:
        ...
        with st.chat_message("assistant"):
            ...
            spinner_text = (
                "Creating your personalized nutrition plan... This may take a minute ⏳"
                ...
                try:
                    ...
                    if nutrition_plan:
                        ...
                        display_response = re.sub(
                            r"```json.*?```", "", response, flags=re.DOTALL | re.IGNORECASE
                        )
                        display_response = re.sub(
                        ...
                        display_response = re.sub(
                            r"in json format\s*[:\.]?", "", display_response, flags=re.IGNORECASE
                        )
                        display_response = re.sub(
                            r"json format\s*[:\.]?", "", display_response, flags=re.IGNORECASE
                        )
                        display_response = re.sub(
                        ...
                        display_response = re.sub(
                            r"below is (the |your )?json", "", display_response, flags=re.IGNORECASE
                        )
                            ...
                            if llm.is_json_truncated(potential_json):
                                ...
                                with st.spinner("Requesting complete plan..."):
                                    try:
                                        ...
                                        nutrition_plan = llm.extract_nutrition_plan(
                                        ...
                                        if nutrition_plan:
                                            ...
                                            retry_display = re.sub(
                                                r"```.*?```", "", retry_display, flags=re.DOTALL
                                            )
                                            retry_display = re.sub(
                                                r"\n\n\n+", "\n\n", retry_display.strip()
                                            )
                                            if retry_display.strip():
                                                ...
                                                st.markdown(
                                                    "I've created your personalized nutrition plan! 🎉"
                                                )
                                        else:
                                            st.error(
                                                "Still couldn't extract complete plan. Please ask me to 'provide a more concise nutrition plan in JSON format'."
                                            )
                    ...
                    if nutrition_plan:
                        ...
                        if st.session_state.get("nutrition_plan_previously_generated"):
                            st.success(
                                f"✅ Nutrition plan updated successfully! ({calories} calories/day)"
                            )
                        else:
                            st.success(
                                f"✅ Nutrition plan generated successfully! ({calories} calories/day)"
                            )
                        ...
                        if has_json_markers:
                            ...
                            if is_incomplete:
                                st.error(
                                    "⚠️ The nutrition plan JSON appears incomplete - the AI response was cut off mid-generation."
                                )
                                st.info(
                                    "💡 **Solution:** Ask me to provide a more concise plan, or ask for just the meal structure without excessive detail."
                                )
                                ...
                                st.warning(
                                    "⚠️ I detected JSON-like content but couldn't extract a valid nutrition plan. The JSON might be missing required fields."
                                )
                                st.info(
                                    "💡 Please ask me to provide the complete nutrition plan in the correct JSON format."
                                )
