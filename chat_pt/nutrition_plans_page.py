...
def render():
    ...
    if not consultations:
        ...
        with col2:
            if st.button(
                "🚀 Start Nutrition Consultation", type="primary", use_container_width=True
            ):
    ...
    with col1:
        ...
        if training_cal and training_cal != "N/A" and rest_cal and rest_cal != "N/A":
            ...
            calories_display = (
                '<div style="font-size: 2rem; font-weight: bold; margin-bottom: 0.5rem;">N/A</div>'
            )
            label = "Daily Calories"
            ...
            if "macros" in nutrition_plan and nutrition_plan["macros"]:
                ...
                if isinstance(macros, dict):
                    ...
                    if "training_day" in macros and isinstance(macros.get("training_day"), dict):
                        training_protein = safe_get_numeric(
                            macros["training_day"], "protein_g", None
                        )
                        rest_protein = None
                        ...
                        if (
                            training_protein
                            and training_protein != "N/A"
                            and rest_protein
                            and rest_protein != "N/A"
                        ):
    ...
    if meal_rotation_notes and meal_rotation_notes != "Not Specified":
        ...
        st.markdown(
            f"<p style='margin: 0;'>{meal_rotation_notes}</p></div>", unsafe_allow_html=True
        )

    # Display meal plans - handle both weekly_plan and days formats
    has_weekly_plan = "weekly_plan" in nutrition_plan and isinstance(
        nutrition_plan["weekly_plan"], dict
    )
                    ...
                    with day_tab:
                        try:
                            ...
                            if not isinstance(day_data, dict):
                                st.warning(
                                    f"Meal data for {display_name} is not in the expected format."
                                )
                                continue
                            ...
                            if not isinstance(meals, list):
                                st.warning(
                                    f"Meals for {display_name} are not in the expected format."
                                )
                                continue
                            ...
                            if meals:
                                ...
                                for i, meal in enumerate(meals, 1):
                                    ...
                                    if calories != "N/A":
                                        protein_html = (
                                            f"<span>P: {protein}g</span>"
                                            if protein != "N/A"
                                            else ""
                                        )
                                        carbs_html = (
                                            f"<span>C: {carbs}g</span>" if carbs != "N/A" else ""
                                        )
                                        fats_html = (
                                            f"<span>F: {fats}g</span>" if fats != "N/A" else ""
                                        )
                            ...
                            st.error(f"Error displaying meals for {display_name}: {e!s}")
        except Exception:
                ...
                for day_tab, (day_name, day_data) in zip(day_tabs, days.items()):
                    with day_tab:
                        try:
                            ...
                            st.error(f"Error displaying meals for {day_name}: {e!s}")
        except Exception:
